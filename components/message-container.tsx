...
const MessageItem: React.FC<{ message: Message; showCitation: (id: string) => void }> = React.memo(({ message, showCitation }) => {
    const replaceCitationFlags = (response: string): JSX.Element => {
        ...
        while ((match = citationRegex.exec(response)) !== null) {
            ...
            if (lastIndex < match.index) {
                parts.push(<span key={lastIndex}>{response.slice(lastIndex, match.index)}</span>);
            }
  ...
  return (
    ...
    >
      <div
        className={`rounded-lg px-4 py-2 max-w-[85%] ${message.role === "user" ? "bg-primary text-primary-foreground" : "bg-muted"}`}
      >
...
const MessageContainer: React.FC<MessageContainerProps> = React.memo(({
  ...
  return (
    <div className="flex-1 overflow-y-auto space-y-4 w-full">
        ...
        {messages.map(
          (message: Message) =>
            message.content && <MessageItem key={message.id} message={message} showCitation={showCitation} />
        )}
      </AnimatePresence>
      ...
      {isLoading && (
        ...
        >
          <div className="flex items-center gap-2 rounded-lg px-4 py-2 bg-muted">
            ...
            <span>
              {toolCall === "getInformation" ? "Getting information..." : "Thinking..."}
            </span>
          </div>
        </motion.div>
      )}
...
(prev, next) =>
    prev.messages === next.messages && prev.isLoading === next.isLoading && prev.error === next.error,
);
