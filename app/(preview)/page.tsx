...
export default function Chat() {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading } =
    useChat({
      ...
      onError: (error) => {
        setError(JSON.parse(error.message)?.error || "An error occurred, please try again later!");
      },
    });
      ...
      <motion.div
        ...
        >
          <div className="flex flex-col w-full justify-between gap-2">
            <form onSubmit={handleSubmitWithErrorReset} className="flex space-x-2">
              <Input
              ...
              <AnimatePresence>
                {error ? (
                  <div className="px-2 min-h-12">
                    <div className="dark:text-red-400 text-red-500 text-sm w-fit mb-1">
                      {error}
                    </div>
                  </div>
